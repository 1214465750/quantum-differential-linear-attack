...
def onedec(p_l,p_r,rk):
    return p_r,rk^p_l^sl(p_r,1)^(sl(p_r,0)&sl(p_r,5))
...
if __name__=='__main__':
    ...
    list_cor.append(NK)
    # print(list_cor)
    np.save('simeck_wrong_0x%x_0x%x_0x%x_0x%x_R%d_RW%d_N%d_NK%d_DKP.npy'%(diff_l,diff_r,lamda_l,lamda_r,round,round_wrong,log2(N),log2(NK)), list_cor)
    t2 = time()
    print('时间',t2-t1)
